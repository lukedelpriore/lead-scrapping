...
**D53. Only credit charged contacts are shown as verified; the Leads page shows the full record.**
While REVEAL_MODE is off, clicking Verify writes a placeholder contact (fixture.<id>@example.com and a 555 number) and spends nothing. These were being labeled Verified, which made fake data look real. Now a contact counts as verified only when creditCharged is true, everywhere: the results table and CSV show the cell, work line, and email only for a real lookup and mark fixtures as New, the verified stat counts only real lookups, and the Leads page lists only credit charged leads with the full record (contact, title, LinkedIn, business, website, city, state, cell, work phone, email, search, delivered date). reveal-batch now targets candidates with no credit charged contact and deletes any prior fixture (and its undelivered lead) before writing, so switching REVEAL_MODE to ask verifies for real and replaces the placeholders cleanly.

**D54. Verify reveals ready candidates regardless of rank.**
reveal-batch only revealed rank primary candidates. When a business's primary owner is already in the lists (a duplicate) and the ready owner is the alternate, the query matched nothing and Verify did nothing even though the UI counted one ready. The rank filter is removed; reveal-batch now reveals ready candidates without a real contact, ordered primary first by rank then by confidence, so the best available ready owner is verified.

**D55. General business discovery uses Google Maps places for real names and addresses.**
Serper web search returned page titles as business names ("Events & Weddings" for a club's events page) and no address. Discovery now uses the Serper places endpoint (Google Maps, same key), which returns the real business name, street address, phone, and website. A new fromSerperPlaces mapping and a parseUsAddress helper (both tested) fill name, address, city, state, main line, and website. Venue gained an address column (migration venue_address). The results table, CSV export, and Leads page show the address. Real business names also make the RocketReach owner search far more accurate. Places coverage is best when the command names a state or metro; a nationwide command returns fewer places.
